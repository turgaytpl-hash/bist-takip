...
def tab_fon_analizi():
    ...
    with t2:
        ...
        # ── HİSSE BAZLI DÖNEM KARŞILAŞTIRMA ──────────────────────────────────
        st.divider()
        st.markdown("### 📊 Hisse Bazlı Dönem Karşılaştırma")
        st.caption("Tüm fonlarda hangi hisseler yeni girdi, hangisi çıktı, kaç fon arttı/azalttı")

        if len(donem_listesi) < 2:
            st.info("Karşılaştırma için en az 2 dönem gerekli.")
        else:
            hk1, hk2 = st.columns(2)
            with hk1:
                hk_yeni = st.selectbox("Yeni dönem:", donem_listesi, index=0, key="hk_yeni")
            with hk2:
                hk_eski_opts = [d for d in donem_listesi if d != hk_yeni]
                hk_eski = st.selectbox("Eski dönem (baz):", hk_eski_opts, key="hk_eski")

            # Her iki dönemin tüm hisselerini çek
            def hisse_fon_map(donem):
                """Dönem → {hisse: set(fonlar)}"""
                sonuc = {}
                for fon_kodu, fon_data in donemler.get(donem, {}).items():
                    for h in fon_data.get('hisseler', []):
                        kod = h['hisse']
                        if kod not in sonuc:
                            sonuc[kod] = {'fonlar': set(), 'agirlik': 0.0, 'deger': 0.0}
                        sonuc[kod]['fonlar'].add(fon_kodu)
                        sonuc[kod]['agirlik'] += h.get('fpd_pct', 0)
                        sonuc[kod]['deger']   += h.get('toplam_deger', 0)
                return sonuc

            yeni_map = hisse_fon_map(hk_yeni)
            eski_map = hisse_fon_map(hk_eski)

            tum_hisseler = set(yeni_map.keys()) | set(eski_map.keys())

            rows_hk = []
            for hisse in sorted(tum_hisseler):
                yeni_bilgi = yeni_map.get(hisse)
                eski_bilgi = eski_map.get(hisse)

                yeni_fon_sayisi = len(yeni_bilgi['fonlar']) if yeni_bilgi else 0
                eski_fon_sayisi = len(eski_bilgi['fonlar']) if eski_bilgi else 0
                delta_fon       = yeni_fon_sayisi - eski_fon_sayisi

                yeni_fonlar = yeni_bilgi['fonlar'] if yeni_bilgi else set()
                eski_fonlar = eski_bilgi['fonlar'] if eski_bilgi else set()
                giren_fonlar = yeni_fonlar - eski_fonlar
                cikan_fonlar = eski_fonlar - yeni_fonlar

                if yeni_fon_sayisi == 0:
                    durum = '🚪 Çıkış'
                elif eski_fon_sayisi == 0:
                    durum = '🆕 Yeni Giriş'
                elif delta_fon > 0:
                    durum = '⬆️ Arttı'
                elif delta_fon < 0:
                ...
                rows_hk.append({
                    'HİSSE'           : hisse,
                    'DURUM'           : durum,
                    f'FON_{hk_yeni}'  : yeni_fon_sayisi,
                    f'FON_{hk_eski}'  : eski_fon_sayisi,
                    'ΔFON'            : delta_fon,
                    'YENİ_GIREN_FONLAR' : ', '.join(sorted(giren_fonlar)) if giren_fonlar else '—',
                    'ÇIKAN_FONLAR'    : ', '.join(sorted(cikan_fonlar))   if cikan_fonlar else '—',
                    f'DEĞER_{hk_yeni}': yeni_bilgi['deger'] if yeni_bilgi else 0,
                })

            df_hk = pd.DataFrame(rows_hk).sort_values(['DURUM', 'ΔFON'], ascending=[True, False])

            # Özet metrikler
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("🆕 Yeni Giriş",  len(df_hk[df_hk['DURUM'] == '🆕 Yeni Giriş']))
            m2.metric("⬆️ Fon Artırdı", len(df_hk[df_hk['DURUM'] == '⬆️ Arttı']))
            m3.metric("⬇️ Fon Azalttı", len(df_hk[df_hk['DURUM'] == '⬇️ Azaldı']))
            m4.metric("🚪 Çıkış",       len(df_hk[df_hk['DURUM'] == '🚪 Çıkış']))

            # Filtre
            filtre = st.radio(
                "Filtrele:",
                ["Tümü", "🆕 Yeni Giriş", "⬆️ Arttı", "⬇️ Azaldı", "🚪 Çıkış"],
                horizontal=True,
                key="hk_filtre",
            )

            df_goster = df_hk if filtre == "Tümü" else df_hk[df_hk['DURUM'] == filtre]

            def renk_hk(row):
                d = row['DURUM']
                if d == '🆕 Yeni Giriş': return ['background-color:#1a5c1a;color:white'] * len(row)
                if d == '⬆️ Arttı':      return ['background-color:#2d7a2d;color:white'] * len(row)
                if d == '⬇️ Azaldı':     return ['background-color:#7a2d2d;color:white'] * len(row)
                if d == '🚪 Çıkış':      return ['background-color:#5c1a1a;color:white'] * len(row)
                return [''] * len(row)

            deger_col = f'DEĞER_{hk_yeni}'
            fmt = {deger_col: '{:,.0f} ₺'}

            st.dataframe(
                df_goster.reset_index(drop=True).style
                    .apply(renk_hk, axis=1)
                    .format(fmt),
                use_container_width=True,
                height=500,
                hide_index=True,
    ...
    with t3:
        ...
        # Karşılaştırılacak dönem — sadece daha eski dönemler (donem_listesi reverse sorted)
        secili_idx = donem_listesi.index(secili_donem)
        diger_donemler = donem_listesi[secili_idx + 1:]
        ...
        if kars_data:
            ...
            for h in fon_data['hisseler']:
                ...
                rows_k.append({
                    'HİSSE'  : kod,
                    'DURUM'  : durum,
                    'BAZ_%'  : ag_e,   # eski dönem
                    'YENİ_%' : ag_y,   # seçili dönem
                    'Δ%'     : d_ag,
                    'YENİ_TL': de_y,
                    'ΔTL'    : de_y - de_e,
                })
            ...
            fmt_all = {**fmt_pct, f'TL_{secili_donem}': '{:,.0f} ₺', 'ΔTL': '{:+,.0f} ₺'}
            ...
            with col_yeni:
                ...
                st.dataframe(
                    df_artan[['HİSSE','DURUM',f'%_{kars_donem}',f'%_{secili_donem}','Δ%']].style.format(fmt_pct),
            ...
            with col_eski:
                ...
                st.dataframe(
                    df_azalan[['HİSSE','DURUM',f'%_{kars_donem}',f'%_{secili_donem}','Δ%']].style.format(fmt_pct),
                    use_container_width=True, hide_index=True,
                )

            st.markdown("#### 📋 Tüm Portföy")
            st.dataframe(
                df_k[['HİSSE','DURUM',f'%_{kars_donem}',f'%_{secili_donem}','Δ%',f'TL_{secili_donem}','ΔTL']].style.format(fmt_all),
                use_container_width=True, hide_index=True,
            )
