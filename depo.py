...
def haftalik_ekle(donem: str,
    ...
    if yab_df is not None and not yab_df.empty:
        ...
        mesajlar.append("ℹ️ Yabancı takas: opsiyonel, atlandı")
...
def aylik_ekle(donem: str,
    ...
    # Pozisyon oranları — sadece gerçek takas verisi varsa hesapla
    if any(df is not None and not (hasattr(df,"empty") and df.empty) for df in [yab_df, fon_df, emk_df]):
